...
import {
  ...
  SnippetString, MarkdownString, Range, EventEmitter,
} from 'vscode';
...
export function encodeDocsUri(query?: Query): Uri {
  return Uri.parse(`al-mobile-helper://search?${JSON.stringify(query)}`);
}
...
export class App {
  ...
  getWebviewContent(query: Query) {
    const config = workspace.getConfiguration('al-mobile-helper');
    const linkUrl = config.get('link-url');
    console.log("%c query", "color:red", query);
    const path = query.path;
    const category = query.category;
    const iframeSrc = `${linkUrl}/#/zh-CN/docs/components/${category}${path}`;
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
export class AlmobileDocsContentProvider implements TextDocumentContentProvider {
...
export class AlmobileCompletionItemProvider implements CompletionItemProvider {
